...
func main() {
	...
	var (
		...
		toolset       = flag.String("toolset", "", "MCP toolset surface: 'full' (default since #2054 — every tool on tools/list) or 'core' (only the 11 loop-essential tools: search, symbol, symbols, context, trace, changes, batch, loop, verify_change, guide, adr). Every tool stays available over HTTP POST /v1/<tool> in both modes; the read-only query set is also reachable as `batch` sub-queries. Falls back to $PINCHER_TOOLSET. The token win rides on PINCHER_SCHEMA_STYLE=lean (still default); the core default was reverted in #2054 because it omitted bootstrap/diagnose-essential tools (index/init/health/...). Set 'core' for token-tight setups. (#2003, #2054)")
	)
...
func printHelpBanner(out io.Writer) {
	...
	fmt.Fprintln(out, "  pincher supervised [--inner-binary PATH]")
	fmt.Fprintln(out, "                                 Run stable MCP stdio provider with auto-respawned inner pincher")
