...
func main() {
	...
	var (
		...
		toolset       = flag.String("toolset", "", "MCP toolset surface: 'core' (default since v1.6 — only the 10 loop-essential tools on tools/list: search, symbol, symbols, context, trace, changes, batch, loop, verify_change, guide) or 'full' (every tool on tools/list). Every tool stays available over HTTP POST /v1/<tool> and as `batch` sub-queries in both modes. Falls back to $PINCHER_TOOLSET. Cuts the per-session tool-schema overhead (measured 3x at scale, #2005); set 'full' + PINCHER_SCHEMA_STYLE=rich to restore the pre-v1.6 surface. (#2003)")
	)
