...
func main() {
	...
	var (
		...
		toolset       = flag.String("toolset", "", "MCP toolset surface: 'core' (default since v1.6 — only the 11 loop-essential tools on tools/list: search, symbol, symbols, context, trace, changes, batch, loop, verify_change, guide, adr) or 'full' (every tool on tools/list). Every tool stays available over HTTP POST /v1/<tool> in both modes; the read-only query set is also reachable as `batch` sub-queries. Falls back to $PINCHER_TOOLSET. Cuts the per-session tool-schema overhead (measured 3x at scale, #2005); set 'full' + PINCHER_SCHEMA_STYLE=rich to restore the pre-v1.6 surface. (#2003)")
	)
