...
func TestOpenStoreReadOnlyOrCreate_MigratesStaleSchema(t *testing.T) {
	...
	// Reset hook_invocations to its pre-v41 shape for the same reason —
	// v40→v41 (hook-redirect-v2) ALTERs in est_tokens_served +
	...
	if _, err := store.DB().Exec(`
		...
		t.Fatalf("create pre-v41 hook_invocations: %v", err)
	}
