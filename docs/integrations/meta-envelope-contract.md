...
```jsonc
{
  ...
  "_meta": {
    ...
    "capabilities": ["schema_v35", "hook_check", "supervised", "..."],
...
| Tag | Meaning |
|---|---|
| `schema_v35` | The DB schema version (current at the time of this release) |
...
def call_pincher(tool, **args):
    ...
    if "schema_v35" not in meta.get("capabilities", []):
