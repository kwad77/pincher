...
```jsonc
{
  ...
  "_meta": {
    ...
    "capabilities": ["schema_v38", "hook_check", "supervised", "..."],
...
| Tag | Meaning |
|---|---|
| `schema_v38` | The DB schema version (current at the time of this release) |
...
def call_pincher(tool, **args):
    ...
    if "schema_v38" not in meta.get("capabilities", []):
