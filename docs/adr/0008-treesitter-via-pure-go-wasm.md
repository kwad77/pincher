...
- **C++ (Phase 2, finale) — the largest absolute gain, and the hardest grammar.** Only language "C++" (`.cpp/.cxx/.cc/.hpp/.hh`) routes to tree-sitter; `.c`/`.h` stay on the regex tier (its grammar + heavy post-processors differ). Two C++-specific recoveries were needed: (1) the **export-macro mis-parse** — `class MODULE_API Name {…}` (Unreal/Win32) is read as a *function_definition* whose return type is a body-less `class_specifier`; recovered by emitting the bare identifier as the Class (and only emitting a class_specifier that has a real `field_declaration_list`, which also drops forward-decls); (2) **constructors/destructors** (no return type) are skipped to match the regex `funcRE` and avoid a Method shadowing its own Class. Namespace-blind moduleQN `::` like the rest; out-of-line `Ret Class::method(){}` binds to its class via the qualifier. Measured on Catch2 (289 files): **13.8% parse errors** — the highest of any language (C++ is the hardest grammar and Catch2 is template/macro-heavy worst-case; graceful regex fallback, no regression), but the gain is enormous because the C/C++ regex tier was barely functional: **287 → 1965 symbols** (regex captured 14 functions and *zero* methods; ts captured 1312 methods + 398 functions), **+1075 #include IMPORTS**, and CALLS **16 → 3407 (213×)**. The all-or-nothing dispatcher means the 86% of files that parse clean get this; the rest are exactly as before. Cost: +~3.5 MB WASM (cpp's 17 MB `parser.c`; total 23.8 MB — the binary is now ~58 MB with all 10 grammars). **Phase 2 complete: PHP, Swift, Ruby, Kotlin, C++ all at AST tier 1.0.**
