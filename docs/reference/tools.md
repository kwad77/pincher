...
> **Schema diet (#2003; toolset default reverted to `full` in #2054):** by default every tool is advertised over MCP `tools/list` with **lean descriptions** (`PINCHER_SCHEMA_STYLE=lean`: descriptions deterministically cut to their first sentence, arg descriptions capped at ~120 chars) — the lean lever carries the dominant token saving (full/lean ≈ 7.1k tools/list tokens vs ≈ 19k full/rich). Set `PINCHER_TOOLSET=core` to narrow the advertisement to the 11 loop-essential tools (`search`, `symbol`, `symbols`, `context`, `trace`, `changes`, `batch`, `loop`, `verify_change`, `guide`, and — since #2020 — `adr`, the cross-session decision store) for token-tight setups. All tools stay registered and reachable over HTTP `POST /v1/<tool>` (and the read-only ones as `batch` sub-queries) in every mode. The v1.6 `core` default was reverted in #2054 because it omitted bootstrap/diagnose-essential tools (`index`, `init`, `health`, `architecture`, `doctor`, …), so a fresh MCP client could neither index nor diagnose a project — and narrowing to core only shaves ~3.7k tokens beyond the lean lever. Set `PINCHER_SCHEMA_STYLE=rich` to restore full descriptions. When a description has been leaned, the full pedagogy for each tool is this document. See [`http-api.md`](http-api.md) → "Server-side env knobs" for the measured token weights.
