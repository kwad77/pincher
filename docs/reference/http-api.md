...
| `PINCHER_TOOLSET` | `full` | Set to `core` (or pass `--toolset core`) at server start to advertise only the 10 loop-essential tools over MCP `tools/list`: `search`, `symbol`, `symbols`, `context`, `trace`, `changes`, `batch`, `loop`, `verify_change`, `guide`. **Every** tool stays registered underneath — HTTP `POST /v1/<tool>` routes, the OpenAPI spec and `batch` sub-query dispatch keep the full surface, and `guide` recommendations that name a full-mode tool carry a restart/HTTP escape-hatch note. Measured (schema-weight gate, #2003): full/rich `tools/list` ≈ 18.4k approx tokens; core ≈ 8.7k; core+lean ≈ 3.0k. Default stays `full` this release; **recommendation**: agents running long delivery loops (where the schema is re-read every turn) should run `core` + `PINCHER_SCHEMA_STYLE=lean` — flipping the default is a future measured decision. |
| `PINCHER_SCHEMA_STYLE` | `rich` | Set to `lean` at server start to apply a deterministic registration-time transform to every tool schema: tool descriptions cut to their first sentence, arg descriptions cut to their first sentence and capped at ~120 chars. Not hand-written variants — one transform over all 34 tools; the pedagogy lives on in `guide` and [`docs/reference/tools.md`](tools.md). Measured: full/lean ≈ 6.9k approx tokens vs 18.4k rich (#2003). Composes with `PINCHER_TOOLSET=core`. |
