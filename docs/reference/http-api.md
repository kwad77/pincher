...
| `PINCHER_TOOLSET` | `core` | Default since v1.6: advertise only the 11 loop-essential tools over MCP `tools/list` — `search`, `symbol`, `symbols`, `context`, `trace`, `changes`, `batch`, `loop`, `verify_change`, `guide`, and (since #2020) `adr`. **Every** tool stays registered underneath — HTTP `POST /v1/<tool>` routes and the OpenAPI spec keep the full surface, `batch` sub-queries additionally dispatch the read-only query set (`search`, `symbol`, `symbols`, `context`, `trace`, `query`, `neighborhood`, `changes` — never write tools), and `guide` recommendations that name a full-mode tool carry a restart/HTTP escape-hatch note. Set to `full` (or pass `--toolset full`) to advertise the complete registered surface — the pre-v1.6 behavior. (The two conditional router proxies, `models`/`route`, are detection-gated, not toolset-gated: see `PINCHER_ROUTER` below.) Measured (schema-weight gate, #2003): full/rich `tools/list` ≈ 18.6k approx tokens; core ≈ 9.2k; core+lean ≈ 3.2k. The flip is the measured decision from the scale round (PR #2005): full/rich burned **1.44M total tokens vs 475k for core+lean** on the same 8-question run at identical 8/8 accuracy — 3.0x waste from the rich schema re-read every turn. |
