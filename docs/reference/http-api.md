...
| `PINCHER_TOOLSET` | `full` | Default since #2054: advertise **every** registered tool over MCP `tools/list`. Set to `core` (or pass `--toolset core`) to narrow the advertisement to the 11 loop-essential tools — `search`, `symbol`, `symbols`, `context`, `trace`, `changes`, `batch`, `loop`, `verify_change`, `guide`, and (since #2020) `adr` — for token-tight setups. Every tool stays registered underneath in both modes — HTTP `POST /v1/<tool>` routes and the OpenAPI spec keep the full surface, `batch` sub-queries additionally dispatch the read-only query set (`search`, `symbol`, `symbols`, `context`, `trace`, `query`, `neighborhood`, `changes` — never write tools), and under `core` the `guide` recommendations that name a non-core tool carry a restart/HTTP escape-hatch note. (The two conditional router proxies, `models`/`route`, are detection-gated, not toolset-gated: see `PINCHER_ROUTER` below.) Why the default reverted to `full` (#2054): the v1.6 `core` default omitted the bootstrap/diagnose-essential tools (`index`, `init`, `health`, `architecture`, `doctor`, …), so a fresh MCP client could neither index nor diagnose a project. The token win was always dominated by the **schema-style** lever, not the toolset lever — measured (schema-weight gate, #2003): full/rich `tools/list` ≈ 19k approx tokens, **full/lean ≈ 7.1k** (the default), core/lean ≈ 3.3k. Narrowing to `core` saves only a further ~3.7k over full/lean — not worth breaking onboarding. |
| `PINCHER_SCHEMA_STYLE` | `lean` | Default since v1.6: a deterministic registration-time transform over every tool schema — tool descriptions cut to their first sentence, arg descriptions cut to their first sentence and capped at ~120 chars. Not hand-written variants — one transform over all 36 tools; the pedagogy lives on in `guide` and [`docs/reference/tools.md`](tools.md). Set to `rich` to restore the full descriptions (pre-v1.6 behavior). Measured: full/lean ≈ 6.9k approx tokens vs 18.4k rich (#2003); the flip rides on the same #2005 scale data as `PINCHER_TOOLSET`. Composes with `PINCHER_TOOLSET`. |
| `PINCHER_ROUTER` | `auto` | pincher-router detection (router-loop B4/B5). `auto` runs the startup ladder — config-dir stat (`~/.config/pincher-router/workers.yaml`) → `pincher-router-serve` on `$PATH` → identity-validated `GET /healthz` (200 + `weights_version`, ≤50ms) — and, when a live router is found, advertises the `router` capability tag **and** the two proxy tools `models`/`route` on `tools/list` (they join the core advertisement; detected-state core+lean ≈ 3.5k approx tokens, still under the 4k budget). Absent ⇒ zero routing surface and zero added schema weight in BOTH toolset modes. `off` disables all routing activity — no probes, no proxy calls (the registered `/v1/models` / `/v1/route` HTTP routes answer with a structured "disabled" error). `on` forces the surface without probing (fixtures/CI). Canonical-value-only: a typo means `off`, never a phantom surface. |
| `PINCHER_ROUTER_ADDR` | `127.0.0.1:7878` | Address the detection probe and the `models`/`route` proxies dial. Proxy calls are bounded at 250ms and never block: an unreachable/slow router yields a structured error telling the loop to proceed at the originating model. |
