...
| `PINCHER_TOOLSET` | `core` | Default since v1.6: advertise only the 10 loop-essential tools over MCP `tools/list` — `search`, `symbol`, `symbols`, `context`, `trace`, `changes`, `batch`, `loop`, `verify_change`, `guide`. **Every** tool stays registered underneath — HTTP `POST /v1/<tool>` routes, the OpenAPI spec and `batch` sub-query dispatch keep the full surface, and `guide` recommendations that name a full-mode tool carry a restart/HTTP escape-hatch note. Set to `full` (or pass `--toolset full`) to advertise the complete registered surface — the pre-v1.6 behavior. (The two conditional router proxies, `models`/`route`, are detection-gated, not toolset-gated: see `PINCHER_ROUTER` below.) Measured (schema-weight gate, #2003): full/rich `tools/list` ≈ 18.4k approx tokens; core ≈ 8.7k; core+lean ≈ 3.0k. The flip is the measured decision from the scale round (PR #2005): full/rich burned **1.44M total tokens vs 475k for core+lean** on the same 8-question run at identical 8/8 accuracy — 3.0x waste from the rich schema re-read every turn. |
| `PINCHER_SCHEMA_STYLE` | `lean` | Default since v1.6: a deterministic registration-time transform over every tool schema — tool descriptions cut to their first sentence, arg descriptions cut to their first sentence and capped at ~120 chars. Not hand-written variants — one transform over all 34 tools; the pedagogy lives on in `guide` and [`docs/reference/tools.md`](tools.md). Set to `rich` to restore the full descriptions (pre-v1.6 behavior). Measured: full/lean ≈ 6.9k approx tokens vs 18.4k rich (#2003); the flip rides on the same #2005 scale data as `PINCHER_TOOLSET`. Composes with `PINCHER_TOOLSET`. |
