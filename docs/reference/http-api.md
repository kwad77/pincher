...
| `PINCHER_TOOLSET` | `core` | Default since v1.6: advertise only the 10 loop-essential tools over MCP `tools/list` — `search`, `symbol`, `symbols`, `context`, `trace`, `changes`, `batch`, `loop`, `verify_change`, `guide`. **Every** tool stays registered underneath — HTTP `POST /v1/<tool>` routes, the OpenAPI spec and `batch` sub-query dispatch keep the full surface, and `guide` recommendations that name a full-mode tool carry a restart/HTTP escape-hatch note. Set to `full` (or pass `--toolset full`) to advertise all 34 tools — the pre-v1.6 behavior. Measured (schema-weight gate, #2003): full/rich `tools/list` ≈ 18.4k approx tokens; core ≈ 8.7k; core+lean ≈ 3.0k. The flip is the measured decision from the scale round (PR #2005): full/rich burned **1.44M total tokens vs 475k for core+lean** on the same 8-question run at identical 8/8 accuracy — 3.0x waste from the rich schema re-read every turn. |
