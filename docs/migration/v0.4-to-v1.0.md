...
| **v0.94 → v0.97+** | Re-install. Schema v36 adds a crash/OOM recovery marker so a killed index pass is retried with a full re-extract instead of trusting partial file hashes. No manual re-index required. v0.97 is launch-prep / CI cleanup only — no tool-schema, DB-schema, or upgrade-action changes. Watch for the v1.0 stable-promotion notes — Phase 5 ships RC fixes only between here and v1.0, and tool schemas freeze at the v1.0 tag. |
...
pincher --version
pincher doctor --json | jq .schema_version  # confirm v36 or newer
