...
// Gated differential test (ADR-0008 Phase 2): real-tree-sitter Swift extractor
// vs the regex tier on a real corpus. Tallies regex and tree-sitter over the
// SAME clean-parse files (the #1967 lesson). Runs only when
// PINCHER_SPIKE_CORPUS_SWIFT points at a dir of .swift files.
...
func TestTreeSitterSwift_DiffVsRegex(t *testing.T) {
	corpus := os.Getenv("PINCHER_SPIKE_CORPUS_SWIFT")
	if corpus == "" {
		t.Skip("set PINCHER_SPIKE_CORPUS_SWIFT=<dir of .swift files> to run the tree-sitter differential")
	}
	var files []string
	_ = filepath.Walk(corpus, func(p string, info os.FileInfo, err error) error {
		if err == nil && !info.IsDir() && strings.HasSuffix(p, ".swift") {
	...
	sort.Strings(files)
	if len(files) == 0 {
		t.Fatalf("no .swift under %s", corpus)
	}

	sx, err := newSwiftTSExtractor(context.Background())
	if err != nil {
	...
	for _, f := range files {
		...
		ts, ok := sx.extractChecked(src, rel)
		if !ok {
			tsErrFiles++
			continue
		}
		rx := extractSwift(src, rel)
	...
	fmt.Fprintf(&b, "\n=== tree-sitter vs regex Swift differential — %s (%d files) ===\n", corpus, len(files))
	...
	fmt.Fprintf(&b, "EDGES\n  IMPORTS regex=%d ts=%d (ts adds import declarations the regex tier never captured)\n  CALLS regex=%d ts=%d\n", rxImports, tsImports, rxCalls, tsCalls)
	fmt.Fprintf(&b, "===========================================================\n")
	t.Log(b.String())
}
