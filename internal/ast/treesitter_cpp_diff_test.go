...
// Gated differential test (ADR-0008 Phase 2): real-tree-sitter C++ extractor
// vs the regex tier on a real corpus, over the SAME clean-parse files. Runs
// only when PINCHER_SPIKE_CORPUS_CPP points at a dir of .cpp/.hpp/.cc files.
...
func TestTreeSitterCpp_DiffVsRegex(t *testing.T) {
	corpus := os.Getenv("PINCHER_SPIKE_CORPUS_CPP")
	if corpus == "" {
		t.Skip("set PINCHER_SPIKE_CORPUS_CPP=<dir of .cpp/.hpp files> to run the tree-sitter differential")
	}
	var files []string
	_ = filepath.Walk(corpus, func(p string, info os.FileInfo, err error) error {
		if err == nil && !info.IsDir() {
			for _, ext := range []string{".cpp", ".cxx", ".cc", ".hpp", ".hh"} {
	...
	cx, err := newCppTSExtractor(context.Background())
	if err != nil {
	...
	for _, f := range files {
		...
		ts, ok := cx.extractChecked(src, rel)
		if !ok {
			tsErrFiles++
			continue
		}
	...
	fmt.Fprintf(&b, "EDGES\n  IMPORTS regex=%d ts=%d (ts adds #include declarations the regex tier never captured)\n  CALLS regex=%d ts=%d\n", rxImports, tsImports, rxCalls, tsCalls)
	fmt.Fprintf(&b, "===========================================================\n")
	t.Log(b.String())
}
