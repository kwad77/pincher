...
// Gated differential test (ADR-0008 Phase 2): real-tree-sitter Ruby extractor
...
// PINCHER_SPIKE_CORPUS_RUBY points at a dir of .rb files.
...
func TestTreeSitterRuby_DiffVsRegex(t *testing.T) {
	corpus := os.Getenv("PINCHER_SPIKE_CORPUS_RUBY")
	if corpus == "" {
		t.Skip("set PINCHER_SPIKE_CORPUS_RUBY=<dir of .rb files> to run the tree-sitter differential")
	}
	var files []string
	_ = filepath.Walk(corpus, func(p string, info os.FileInfo, err error) error {
		if err == nil && !info.IsDir() && strings.HasSuffix(p, ".rb") {
	...
	sort.Strings(files)
	if len(files) == 0 {
		t.Fatalf("no .rb under %s", corpus)
	}

	rx2, err := newRubyTSExtractor(context.Background())
	if err != nil {
	...
	for _, f := range files {
		...
		ts, ok := rx2.extractChecked(src, rel)
		if !ok {
			tsErrFiles++
			continue
		}
		rx := extractRuby(src, rel)
	...
	fmt.Fprintf(&b, "\n=== tree-sitter vs regex Ruby differential — %s (%d files) ===\n", corpus, len(files))
	...
	fmt.Fprintf(&b, "EDGES\n  IMPORTS regex=%d ts=%d (ts adds require declarations the regex tier never captured)\n  CALLS regex=%d ts=%d\n", rxImports, tsImports, rxCalls, tsCalls)
	fmt.Fprintf(&b, "===========================================================\n")
	t.Log(b.String())
}
