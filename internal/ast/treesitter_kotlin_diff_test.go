...
// Gated differential test (ADR-0008 Phase 2): real-tree-sitter Kotlin extractor
// vs the regex tier on a real corpus. Tallies over the SAME clean-parse files
// (the #1967 lesson). Runs only when PINCHER_SPIKE_CORPUS_KOTLIN points at a
// dir of .kt files.
...
func TestTreeSitterKotlin_DiffVsRegex(t *testing.T) {
	corpus := os.Getenv("PINCHER_SPIKE_CORPUS_KOTLIN")
	if corpus == "" {
		t.Skip("set PINCHER_SPIKE_CORPUS_KOTLIN=<dir of .kt files> to run the tree-sitter differential")
	}
	var files []string
	...
	sort.Strings(files)
	if len(files) == 0 {
		t.Fatalf("no .kt under %s", corpus)
	}

	kx, err := newKotlinTSExtractor(context.Background())
	if err != nil {
	...
	for _, f := range files {
		...
		ts, ok := kx.extractChecked(src, rel)
		if !ok {
			tsErrFiles++
			continue
		}
		rx := extractKotlin(src, rel)
	...
	fmt.Fprintf(&b, "\n=== tree-sitter vs regex Kotlin differential — %s (%d files) ===\n", corpus, len(files))
