...
//     denominator.
//   - HookTokenColumnsPresent gates the v40 #1983 token-estimate columns:
//     false on the shipped schema, true after the columns are added, and
//     HookRedirectTokensLeftOnTable then sums the measured estimates.
...
func TestHookTokenColumns_GateAndPricedSum(t *testing.T) {
	...
	// Shipped schema (pre-v40): the gate must report absent.
	if s.HookTokenColumnsPresent() {
		t.Fatal("HookTokenColumnsPresent = true on the shipped schema — gate would price from columns that don't exist")
	}

	// Simulate the v40-on-#1983 shape and verify the priced path.
	for _, ddl := range []string{
		`ALTER TABLE hook_invocations ADD COLUMN est_tokens_served INTEGER NOT NULL DEFAULT 0`,
		`ALTER TABLE hook_invocations ADD COLUMN est_tokens_baseline INTEGER NOT NULL DEFAULT 0`,
	} {
		if _, err := s.db.Exec(ddl); err != nil {
			t.Fatalf("simulate v40 columns: %v", err)
		}
	}
	if !s.HookTokenColumnsPresent() {
		t.Fatal("HookTokenColumnsPresent = false after adding both columns")
	}

	now := time.Now().UnixNano()
	sid := "sess-priced"
	if _, err := s.db.Exec(
		`INSERT INTO hook_invocations(ts, session_id, tool_name, decision, suggested_tool, took_recommendation, est_tokens_served, est_tokens_baseline)
