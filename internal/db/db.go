...
END;`,
	...
	// v39 → v40: hook redirect savings telemetry (hook-redirect-v2).
...
var schemaMigrationInvalidates = []MigrationInvalidates{
	...
	invalidatesNothing, // [38] v39→v40: hook_invocations.est_tokens_served + baseline_tokens (telemetry only; pre-migration rows hold NULL)
}
...
func (s *Store) traceViaCTE(projectID, startID, direction string, edgeKinds []string, maxDepth int) ([]TraceResult, error) {
	...
	runDir := func(dir string) error {
		...
		q := `WITH RECURSIVE reach(id, depth, via) AS (
			...
			FROM reach r
			JOIN edges e` + indexHint + ` ON ` + joinCond + ` AND e.kind IN (` + in + `)` + projectFilter + `
			WHERE r.depth < ?
		)
...
type HookInvocation struct {
	...
	// EstTokensServed / BaselineTokens (v40, hook-redirect-v2) back the
...
// HookSavings7d sums the v40 per-redirect savings telemetry over the
...
// recorded baseline contribute — pre-v40 rows hold NULL and are
