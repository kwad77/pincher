...
//     and bypassed (took_recommendation = 0). Priced from the v41
//     hook-redirect-v2 baseline_tokens/est_tokens_served columns when
//     present (the gate discovers them via PRAGMA, so on this tree the
//     priced path is live); on a pre-v41 schema the finding degrades to
...
// the agent saw and bypassed. Token pricing is gated on the v41
// hook-redirect-v2 per-row estimate columns; without them (pre-v41
// schema) the finding is counts-only — a real count beats an invented
// token figure.
func (s *Server) coachHookFallThrough(window string, since time.Time) map[string]any {
	...
	if s.store.HookTokenColumnsPresent() {
		...
		basis = fmt.Sprintf(
			"%d of %d resolved redirect(s) in %s were bypassed (took_recommendation = 0; %d redirect(s) total, unresolved ones excluded). est = Σ max(baseline_tokens − est_tokens_served, 0) over the bypassed rows = %d — per-row estimates recorded by the hook at intercept time.",
			ignored, resolved, scope, redirects, est)
	} else {
		basis = fmt.Sprintf(
			"%d of %d resolved redirect(s) in %s were bypassed (took_recommendation = 0; %d redirect(s) total, unresolved ones excluded). This schema predates the per-row token-estimate columns (est_tokens_served/baseline_tokens, v41), so the token figure is 0 — counts-only coaching, not an invented number.",
