...
func seedAllPatterns(t *testing.T, srv *Server, store *db.Store) {
	...
	// Carries the v41 per-row token estimates so the priced path has
	// measured numbers to sum: baseline 1500 − served 200 = 1300.
	...
	if err := store.LogHookInvocation(db.HookInvocation{
		...
		EstTokensServed: 200, BaselineTokens: 1500,
	}); err != nil {
...
func TestCoach_AllShippedPatternsFireWithDocumentedPrices(t *testing.T) {
	...
	// 5. hook_fall_through: priced on the shipped v41 schema — the
	// PRAGMA gate discovers est_tokens_served/baseline_tokens and sums
	// the per-row estimates instead of degrading to counts-only.
	...
	if got, _ := hook["est_tokens_left_on_table"].(float64); got != 1300 {
		t.Errorf("hook est = %v, want 1300 (max(baseline 1500 − served 200, 0) from the seeded redirect)", got)
	}
	if basis, _ := hook["basis"].(string); !strings.Contains(basis, "per-row estimates recorded by the hook") {
		t.Errorf("hook basis %q should document the priced per-row-estimate path", basis)
	}
}
...
func TestCoach_LimitKeepsBiggestMeasuredWins(t *testing.T) {
	...
	// Sorted by est desc: heavy (6400) must be first; on the v41 schema
	// the hook finding is priced (1300, from the seeded per-row
	// estimates) so it takes the second slot, ahead of churn (900).
	...
	if second["pattern"] != "hook_fall_through" {
		t.Errorf("second finding = %v, want hook_fall_through (priced at 1300 on the v41 schema)", second["pattern"])
	}
	if f := findingByPattern(t, body, "zero_result_churn"); f != nil {
		t.Error("zero_result_churn (est 900) survived limit=2 — sort order broken")
	}
}
