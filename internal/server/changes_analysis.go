...
type ChangesAnalysis struct {
	...
	// or the diff would be apples-vs-orchard); handleChanges and the
	// test-impacted CLI ignore it.
...
// the `verify_change` post-edit gate (loop-substrate PR-10), and the
// `pincher test-impacted` CLI. Extracted from handleChanges so every
// consumer executes the SAME analysis the agent sees, instead of
// forking the logic. (The loop-substrate wave and the test-impacted
// branch each extracted this independently — analyzeChanges in
// verify_change.go and AnalyzeChanges here; the integration pass
// reconciled them into this single core, which carries the union:
// the ChangeTracer seam + typed results from the test-impacted
// extraction, DirectCallers from the verify-change one.)
//
...
// best-effort, matching handleChanges' historical behaviour. The error
// is returned unwrapped — each consumer adds its own "git diff failed"
// framing to its envelope.
