...
// (runtime gates included; since the loop-substrate integration the
// production tool surface registers batch and loop alongside the
// three composites).
func TestComputeGuide_ShapeRouting_FirstRecommendation(t *testing.T) {
	...
	cases := []struct {
		...
		// The loop tool is registered post-integration, but a fresh
		// test store has an empty ledger → resumption still falls
		// back to adr list (ledger gate).
		{"continue where I left off", shapeResume, "adr"},
		// batch IS registered post-integration (meta-max-tokens) →
		// multi-question tasks route to the batch composite. (On the
		// original guide-coaching branch batch was unregistered and
		// this fell back to the keyword path → understand/architecture;
		// that claim did not survive integration.)
		{"what is the indexer? how does search work?", shapeBatch, "batch"},
...
// registered loop tool and a non-empty loop ledger. A fresh store
// migrates loop_checkpoints (schema v40, loop-substrate integration)
// but the ledger holds zero rows, so even with the tool registered the
// recommendation stays adr list.
